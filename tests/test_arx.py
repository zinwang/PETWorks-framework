...
def arxDataAdult(DATASET_PATH_ADULT) -> Data:
    return loadDataFromCsv(DATASET_PATH_ADULT["originalData"], StandardCharsets.UTF_8, ";")
...
def testSetDataHierarchies(
    ...
    assert dataDefinition.getAttributeType("education").toString() == "IDENTIFYING_ATTRIBUTE"
    assert dataDefinition.getAttributeType("race").toString() == "INSENSITIVE_ATTRIBUTE"
    assert dataDefinition.getAttributeType("salary-class").toString() == "INSENSITIVE_ATTRIBUTE"
    assert dataDefinition.getAttributeType("sex").toString() == "INSENSITIVE_ATTRIBUTE"
    assert dataDefinition.getAttributeType("workclass").toString() == "INSENSITIVE_ATTRIBUTE"
